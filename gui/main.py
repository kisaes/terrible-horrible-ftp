...
class MainWindow(QMainWindow):
    ...
    def connect_to_server(self):
        if self.clientThread:
            if QMessageBox.question(self, "Connected already",  "Are you sure to close current connection?", QMessageBox.Yes | QMessageBox.No) == QMessageBox.Yes:
    ...
    def setRemotedir(self, remote_dir):
        self.remote_dir = remote_dir if remote_dir != '/' else ''
    ...
    def errorSlot(self, code, detail):
        if code in [-1, 421]:
            self.disconnect_from_server()
        self.ui.statusbar.showMessage('%d %s' % (code, detail))
